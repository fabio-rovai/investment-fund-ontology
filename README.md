...
| 3 | ETF funds whose share class has no listing anywhere in public data: the single Violation-severity rule in the ontology's own SHACL layer (every other finding here is Warning-severity) | **73 findings across 71 of 4,053 ETF funds (1.8%)** |
...
### Independent replication of the layer-3 rules

The four layer-3 rules have been executed *as written* — the plain SPARQL inside their `sh:select` bodies, no set-based re-implementation — on the [Open Ontologies](https://github.com/fabio-rovai/open-ontologies) Oxigraph engine, against the full 1.29M-triple graph: load plus all four rules in 2.1 seconds, counts identical to this pipeline's published figures (73 / 0 / 214 / 2,148). The same experiment measured rdflib 7.6.0 on the same graph: the anti-join (R1) and nested aggregate (R5) complete in under a second, but both multi-way self-joins (R2, R3) exceed a 300-second timeout — which is why this repo's reference pipeline executes the rules set-based in Python. Details and a runnable demo: [the IFO case study in Open Ontologies](https://github.com/fabio-rovai/open-ontologies/tree/main/case-studies/investment-fund-ontology).
