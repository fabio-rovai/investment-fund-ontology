...
The zero checksum-failure results on the GLEIF file are guarded against a vacuous validator: `pipeline/checksums.py` embeds negative test vectors (corrupted Apple ISIN, LEI, CUSIP; corrupted HSBC SEDOL) and the self-test runs them on every invocation. The 19 LEI check-digit failures found inside N-CEN filings come from the same code paths that pass 9.1 million GLEIF LEIs, which is what makes them credible.

## What could NOT be obtained from public data

Stated plainly, because each gap is itself a finding:

1. **Venue for 96.8 percent of quotations.** The SEC series/class register publishes tickers with no venue field. Only the 1,130 ETF listings joinable to N-CEN exchange rows resolve to a MIC.
2. **Class-level ISIN resolution.** GLEIF maps LEI to ISIN set; the LEI sits at fund level. Resolving which ISIN belongs to which share class requires CUSIP-to-class data, which is licensed (CUSIP Global Services). 259 funds resolve exactly by unique pairing (one class, one ISIN); the rest stay honestly at fund level via `ifo:hasIssuedSecurityIdentifier`.
3. **ISINs for 87.7 percent of ETFs.** Only 497 of 4,053 ETF funds have any ISIN against their LEI in the GLEIF open mapping, although every one of them is exchange-traded and therefore has an ISIN in commercial datasets.
